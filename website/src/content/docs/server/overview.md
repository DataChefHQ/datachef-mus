...
The browser can't call the Slack API directly — it would expose your bot token. MUS routes all Slack communication through a server-side handler that reads `SLACK_BOT_TOKEN` from the environment.
